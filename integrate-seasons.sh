#!/usr/bin/env bash
# Integra popup10-1.js (selettore stagioni/episodi) nel plugin
# Owlfin, come quinta funzione attivabile separatamente dalle altre.
#
# USO:
#   1) Copia questo file nella cartella del plugin, es:
#        ~/jellyplugin/Owlfin/integrate-seasons.sh
#   2) Dalla cartella del plugin:
#        chmod +x integrate-seasons.sh
#        ./integrate-seasons.sh patch
#      Questo scrive Web/seasons.js e modifica con sed csproj, config,
#      TransformationPatches.cs, configPage.html, e i tre build*.yaml.
#   3) Compila per entrambi i target (come fai di solito), es:
#        cp build-10.11.yaml build.yaml && ./build.sh
#        cp build-12.0.yaml  build.yaml && ./build.sh
#      Segnati i due checksum md5 da dist/owlfin_*.zip.md5sum.
#   4) Aggiorna i due manifest.json con i checksum reali:
#        ./integrate-seasons.sh manifest <checksum-10.11> <checksum-12.0>
#   5) Rivedi il diff, poi carica su GitHub:
#        git add -A
#        git commit -m "Aggiunto selettore stagioni ed episodi (seasons.js)"
#        git push
#
# Va eseguito UNA VOLTA sola su un checkout pulito (i sed cercano testo che
# esiste solo prima della patch). Se qualcosa fallisce a metà, fai
# "git checkout -- ." e riparti.

set -euo pipefail

VERSION_1011="1.5.0.0"
VERSION_120="2.2.0.0"

if [[ ! -f Plugin.cs || ! -d Web ]]; then
  echo "Esegui questo script dalla cartella del plugin Owlfin (dove c'e' Plugin.cs)." >&2
  exit 1
fi
...
if [[ "$cmd" == "manifest" ]]; then
  CHECKSUM_1011="${2:?Uso: ./integrate-seasons.sh manifest <checksum-10.11> <checksum-12.0>}"
  CHECKSUM_120="${3:?Uso: ./integrate-seasons.sh manifest <checksum-10.11> <checksum-12.0>}"
  TS="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

  python3 - "$VERSION_1011" "$CHECKSUM_1011" "$TS" << 'PYEOF'
import json, sys
version, checksum, ts = sys.argv[1], sys.argv[2], sys.argv[3]
path = "10.11/manifest.json"
with open(path) as f:
    data = json.load(f)
entry = {
    "version": version,
    "changelog": f"{version}: allineata alla 2.2.0.0 (stesso seasons.js): selettore stagioni/episodi con segna come visto automatico.\n",
    "targetAbi": "10.11.0.0",
    "sourceUrl": f"https://raw.githubusercontent.com/whoissava/owlfin/main/10.11/owlfin/owlfin_{version}.zip",
    "checksum": checksum,
    "timestamp": ts,
}
...
with open(path, "w") as f:
    ...
    f.write("\n")
print(f"Aggiornato {path} con la versione {version}")
PYEOF

  python3 - "$VERSION_120" "$CHECKSUM_120" "$TS" << 'PYEOF'
import json, sys
version, checksum, ts = sys.argv[1], sys.argv[2], sys.argv[3]
path = "12.0/manifest.json"
with open(path) as f:
    data = json.load(f)
entry = {
    "version": version,
    "changelog": f"{version}: aggiunto il selettore stagioni/episodi nella pagina serie (nuovo file seasons.js), con segna come visto automatico degli episodi precedenti; attivabile separatamente dalle altre funzioni.\n",
    "targetAbi": "12.0.0.0",
    "sourceUrl": f"https://raw.githubusercontent.com/whoissava/owlfin/main/12.0/owlfin/owlfin_{version}.zip",
    "checksum": checksum,
    "timestamp": ts,
}
data[0]["versions"].insert(0, entry)
...
PYEOF
  ...
  echo "Fatto. Controlla 'git diff 10.11/manifest.json 12.0/manifest.json' prima di committare."
  exit 0
fi

if [[ "$cmd" != "patch" ]]; then
  echo "Uso: ./integrate-seasons.sh patch          (prima passata: scrive il codice)"
  echo "     ./integrate-seasons.sh manifest A B    (dopo la build: aggiorna i manifest)"
  exit 1
fi

echo "==> Scrivo Web/seasons.js"
cat > Web/seasons.js << 'OWLFIN_SEASONS_JS_EOF'
(function () {
  'use strict';
...
OWLFIN_SEASONS_JS_EOF

echo "==> Aggiungo Web/seasons.js come EmbeddedResource nel .csproj"
sed -i '/<EmbeddedResource Include="Web\\chat.js" \/>/a\    <None Remove="Web\\seasons.js" />\n    <EmbeddedResource Include="Web\\seasons.js" />' Jellyfin.Plugin.Owlfin.csproj

echo "==> Aggiungo la proprieta' SeasonsEnabled alla configurazione"
sed -i '/^        ChatEnabled = true;$/a\        SeasonsEnabled = false;' Configuration/PluginConfiguration.cs
sed -i '/^    public bool ChatEnabled { get; set; }$/a\
\
    /// <summary>\
    /// Gets or sets a value indicating whether the season/\
    /// episode popup (seasons.js) is injected into the web client. Restart\
    /// Jellyfin after changing this.\
    /// <\/summary>\
    public bool SeasonsEnabled { get; set; }' Configuration/PluginConfiguration.cs

echo "==> Aggiungo le proprieta' mancanti TrendingEnabled/TmdbApiKey (gia' presenti in configPage.html ma senza backing in C#)"
sed -i '/^        SonarrApiKey = string.Empty;$/a\        TrendingEnabled = false;\n        TmdbApiKey = string.Empty;' Configuration/PluginConfiguration.cs
sed -i '/^    public string SonarrApiKey { get; set; }$/a\
\
    /// <summary>\
    /// Gets or sets a value indicating whether the trending Top 10 (TMDB)\
    /// banner is shown on the Home screen. Requires a TMDB API key.\
    /// <\/summary>\
    public bool TrendingEnabled { get; set; }\
\
    /// <summary>\
    /// Gets or sets the TMDB API key used to fetch the trending Top 10.\
    /// Used only in server-side requests -- never sent to the browser.\
    /// <\/summary>\
    public string TmdbApiKey { get; set; }' Configuration/PluginConfiguration.cs

echo "==> Aggiungo l'iniezione condizionale in TransformationPatches.cs"
sed -i '/scripts += BuildScriptElement("Owlfin-Chat", "Web.chat.js");/{n;a\
\
...
echo "==> Aggiungo il checkbox e le righe JS in configPage.html"
sed -i '/I messaggi sono visibili a tutti gli utenti del server\./{n;n;a\
\
                    <div class="checkboxContainer checkboxContainer-withDescription">\
                        <label>\
                            <input id="SeasonsEnabled" name="SeasonsEnabled" type="checkbox" is="emby-checkbox" />\
                            <span>Selettore stagioni ed episodi</span>\
                        </label>\
                        <div class="fieldDescription checkboxFieldDescription">\
                            Sulla pagina serie sostituisce il blocco Stagioni nativo con un\
                            menu a comparsa e una griglia episodi, con segna\
                            come visto automatico degli episodi precedenti.\
                        </div>\
                    </div>
}' Configuration/configPage.html
sed -i "/document.querySelector('#ChatEnabled').checked = config.ChatEnabled;/a\\                        document.querySelector('#SeasonsEnabled').checked = config.SeasonsEnabled;" Configuration/configPage.html
sed -i "/config.ChatEnabled = document.querySelector('#ChatEnabled').checked;/a\\                        config.SeasonsEnabled = document.querySelector('#SeasonsEnabled').checked;" Configuration/configPage.html

echo "==> Aggiorno le versioni nei build*.yaml"
sed -i 's/^version: "1\.3\.4\.0"$/version: "'"${VERSION_1011}"'"/' build.yaml build-10.11.yaml
sed -i 's/^version: "2\.0\.7\.0"$/version: "'"${VERSION_120}"'"/' build-12.0.yaml

echo "==> Fatto. File toccati:"
echo "    Web/seasons.js (nuovo)"
echo "    Jellyfin.Plugin.Owlfin.csproj"
echo "    Configuration/PluginConfiguration.cs"
echo "    Helpers/TransformationPatches.cs"
echo "    Configuration/configPage.html"
echo "    build.yaml, build-10.11.yaml, build-12.0.yaml (versione -> ${VERSION_1011} / ${VERSION_120})"
echo
echo "NB: ho aggiunto anche TrendingEnabled/TmdbApiKey a PluginConfiguration.cs"
echo "(il checkbox e il campo esistevano gia' in configPage.html ma senza"
echo "proprieta' corrispondente, quindi il salvataggio veniva ignorato)."
echo "Lo script Web/trending.js e il controller TMDB citati nel changelog"
echo "non esistono pero' nel codice: la checkbox ora si salva, ma il banner"
echo "Top 10 non fa ancora nulla finche' non si scrive quella parte."
echo
echo "Ora compila per i due target e poi lancia:"
echo "    ./integrate-seasons.sh manifest <checksum-10.11> <checksum-12.0>"
